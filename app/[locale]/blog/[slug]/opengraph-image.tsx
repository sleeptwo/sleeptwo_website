...
const categoryColors: Record<string, { pill: string; text: string; glow: string }> = {
  ...
  "Apple Watch":          { pill: "rgba(14,165,233,0.25)",  text: "#38bdf8", glow: "rgba(14,165,233,0.13)" },
  "Health & HRV":         { pill: "rgba(16,185,129,0.25)",  text: "#34d399", glow: "rgba(16,185,129,0.13)" },
...
export default function Image({ params }: { params: { locale: Locale; slug: string } }) {
  const post = getLocalizedPost(params.slug, params.locale);
  const cat = post ? (categoryColors[post.category] ?? fallback) : fallback;
  const title = post?.title ?? "SleepTwo Blog";
  const category = post?.category ?? "Article";
  const fontSize = title.length > 60 ? 46 : title.length > 45 ? 54 : 62;
  ...
  return new ImageResponse(
    (
      ...
      >
        {/* Background glow */}
        <div style={{ position: "absolute", width: 800, height: 800, borderRadius: "50%", background: cat.glow, filter: "blur(140px)", bottom: -350, right: -250 }} />
        <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", background: "rgba(124,58,237,0.1)", filter: "blur(80px)", top: -150, left: -100 }} />

        {/* Top row: category pill */}
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ background: cat.pill, color: cat.text, fontSize: 18, fontWeight: 600, padding: "9px 22px", borderRadius: 100, letterSpacing: 0.3 }}>
            {category}
          </div>
        ...
        {/* Title */}
        <div style={{ fontSize, fontWeight: 700, color: "white", letterSpacing: -1, lineHeight: 1.2, maxWidth: 1000 }}>
          {title}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 11, background: "linear-gradient(135deg, #7C3AED, #DB2777)", display: "flex", alignItems: "center", justifyContent: "center" }} />
            <span style={{ color: "rgba(255,255,255,0.8)", fontSize: 24, fontWeight: 600, letterSpacing: -0.5 }}>SleepTwo</span>
          </div>
          <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 20 }}>sleeptwo.app/blog</span>
        </div>
      </div>
    ),
    { ...size }
