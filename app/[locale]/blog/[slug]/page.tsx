...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: {
        en: `https://sleeptwo.app/en/blog/${slug}`,
        "zh-Hant": `https://sleeptwo.app/zh-Hant/blog/${slug}`,
        "zh-Hans": `https://sleeptwo.app/zh-Hans/blog/${slug}`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.description,
      url: `https://sleeptwo.app/${locale}/blog/${slug}`,
      type: "article",
...
export default async function PostPage({
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: `https://sleeptwo.app/${locale}` },
      { "@type": "ListItem", position: 2, name: tr.blog.title, item: `https://sleeptwo.app/${locale}/blog` },
      { "@type": "ListItem", position: 3, name: post.title, item: `https://sleeptwo.app/${locale}/blog/${slug}` },
    ],
  };
      ...
      <main style={{ background: "var(--bg)", minHeight: "100vh" }}>
        ...
        <div className="relative w-full h-72 md:h-96 overflow-hidden">
          ...
          <div
            ...
            style={{ background: "linear-gradient(to bottom, rgba(7,7,15,0.3) 0%, rgba(7,7,15,0.85) 100%)" }}
          />
            ...
            <div className="max-w-3xl mx-auto px-6 pb-10 w-full">
              ...
              <div className="flex items-center gap-3 mb-3">
                <span
                  ...
                  style={{ background: "rgba(124,58,237,0.25)", color: "var(--pa)", border: "1px solid rgba(124,58,237,0.4)" }}
                >
                  {post.category}
                </span>
                <span className="text-xs" style={{ color: "var(--muted)" }}>
                  {post.readTime} {tr.blog.minRead}
                </span>
                <span className="text-xs" style={{ color: "var(--muted)" }}>
        ...
        <article className="max-w-3xl mx-auto px-6 py-12">
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <p className="text-sm font-semibold" style={{ color: "var(--text)" }}>SleepTwo Team</p>
              <p className="text-xs" style={{ color: "var(--muted)" }}>
                {new Date(post.publishedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })} · {post.readTime} {tr.blog.minRead}
              </p>
            </div>
          </div>
              ...
              {sources.map((s) => (
                <li key={s.url} className="flex items-start gap-2 text-sm">
                  ...
                  <span>
                    ...
                    <span className="ml-1.5 text-xs" style={{ color: "rgba(255,255,255,0.25)" }}>— {s.source}</span>
                  </span>
                </li>
              ))}
            </ul>
          ...
          {/* Download CTA */}
          <div className="mt-16 rounded-3xl p-10 text-center glass" style={{ border: "1px solid var(--border)" }}>
            <div className="flex justify-center mb-4">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/icon.png" alt="SleepTwo" className="w-16 h-16 rounded-2xl" />
            </div>
            <h3 className="text-xl font-bold mb-2" style={{ color: "var(--text)" }}>{tr.download.h1}</h3>
            <p className="mb-6 text-sm" style={{ color: "var(--muted)" }}>
              {tr.download.desc.split("\n")[1]}
            </p>
